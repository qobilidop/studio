...
- ARCHITECTURE-NEXT: separate core Rust lib from WASM bindings, diff testing (Rust sim vs Sail C sim), proptest→Isla→formal verification pipeline (Thu)
...
- THEME: Mon = theory (SMT, Zig), Tue = project bootstrap (sail-xisa), Wed = massive build sprint (ISA completion + website), Thu = web playground build + polish + spec page
