...
Quarter opens with rapid sail-xisa progress (full Parser ISA, MAP ISA, web playground, then Cargo workspace refactor + proptest + diff testing + bug fixes). Z3Wire gains multiplication, gets a major docs/dev refactor, adds 6 design decision records and gate-level examples. P4Py tor.p4 translation completed. Personal website launched. Deep exploration of agentic dev workflow standards. W15 continues with sail-xisa MAP ISA completion (all 44 self-contained opcodes done), Zensical adoption, and Rust+MLIR/BPF ecosystem research.
...
- [W15](w15/README.md): Zensical deep dive + Z3Wire tooling migration (MkDocs Material to Zensical, pip to uv), sail-xisa docs restructuring + MAP ISA completion (opcodes 26-43, all self-contained done), Rust+MLIR/BPF/P4 backend exploration
...
- SAIL-XISA: formal XISA spec in Sail — Parser ISA complete (42 variants), MAP ISA started (28 variants W14) then completed (44 opcodes, all self-contained done W15), 64-bit binary encoding, web playground (Rust/WASM) + /spec page, Cargo workspace refactor, proptest + diff testing (116 Rust tests), bit-endianness bug cascade fixed. Docs restructured (Starlight reverted, spec/plans deleted, docs/ flattened). Strategy: Sail as ground truth oracle. XISA spec MPLv2, project Apache 2.0 (W14, W15)
